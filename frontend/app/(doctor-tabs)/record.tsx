...
import { Mic, MicOff, Lock, Cpu, Square, Edit3, Clock, BarChart3, Send, CheckCircle, AlertTriangle, Stethoscope, Pill, Activity, FileText, Brain, Thermometer, ChevronDown, ChevronUp, Globe, Languages, RefreshCw, User, Save, ClipboardList } from 'lucide-react-native';
...
const CONSULTATION_TYPES = [
  { key: 'general', label: 'General Visit', icon: '🏥' },
  { key: 'follow_up', label: 'Follow-up', icon: '🔄' },
  { key: 'new_patient', label: 'New Patient', icon: '👤' },
...
export default function DoctorRecordScreen() {
  ...
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    symptoms: true, diagnosis: true, medications: true, history: false, vitals: false, tests: false, quotes: false
  });
  ...
  const requestMicPermission = async () => {
    try {
      ...
      if (!granted) {
        Alert.alert('Microphone Permission Required', 'MedScribe needs microphone access to record clinic visit conversations.');
      }
  ...
  const selectedTypeInfo = CONSULTATION_TYPES.find(t => t.key === consultationType) || CONSULTATION_TYPES[0];
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      if (Platform.OS !== 'web') {
        Alert.alert('Recording Failed', 'Could not start recording. Please ensure microphone permission is granted.');
      }
  ...
  const retryUpload = () => {
    if (lastAudioUri) { uploadAndTranscribe(lastAudioUri); }
  };
  ...
  const saveConsultation = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${BACKEND_URL}/api/doctor/consultation`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        ...
        setError(e.detail || 'Failed to save consultation');
        setStep('review');
      }
    } catch (e) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={st.flex}>
        <ScrollView contentContainerStyle={st.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={st.headerRow}>
            <Text style={st.title}>Clinic Visit</Text>
          ...
          <View style={st.configRow}>
            <TouchableOpacity style={st.languageSelector} onPress={() => setShowLanguagePicker(true)} activeOpacity={0.7}>
              <Languages size={14} color="#0033A0" />
              <Text style={st.langText}>{selectedLangInfo.flag} {selectedLangInfo.name}</Text>
              <ChevronDown size={12} color={theme.textSecondary} />
            </TouchableOpacity>
            <TouchableOpacity style={st.typeSelector} onPress={() => setShowTypePickerModal(true)} activeOpacity={0.7}>
              <Text style={st.typeIcon}>{selectedTypeInfo.icon}</Text>
              <Text style={st.typeText}>{selectedTypeInfo.label}</Text>
              <ChevronDown size={12} color={theme.textSecondary} />
            </TouchableOpacity>
          </View>
          ...
          <Modal visible={showLanguagePicker} transparent animationType="slide">
            <TouchableOpacity style={st.modalOverlay} activeOpacity={1} onPress={() => setShowLanguagePicker(false)}>
              <View style={st.modalContent}>
                ...
                <Text style={st.modalDesc}>Choose the language of the clinic conversation.</Text>
                <FlatList
                  data={languages}
                  ...
                  renderItem={({ item }) => (
                    ...
                    >
                      ...
                      <View style={st.langInfo}>
                        ...
                        <Text style={st.langCode}>{item.code === 'auto' ? 'Auto-detects language' : `Code: ${item.code}`}</Text>
                      </View>
                      {selectedLanguage === item.code && <CheckCircle size={18} color="#0033A0" />}
                    </TouchableOpacity>
                  )}
                />
              </View>
          ...
          <Modal visible={showTypePickerModal} transparent animationType="slide">
            <TouchableOpacity style={st.modalOverlay} activeOpacity={1} onPress={() => setShowTypePickerModal(false)}>
              <View style={st.modalContent}>
                <Text style={st.modalTitle}>Consultation Type</Text>
                <Text style={st.modalDesc}>Categorize this clinic visit.</Text>
                {CONSULTATION_TYPES.map(t => (
                  <TouchableOpacity
                    key={t.key}
                    style={[st.langOption, consultationType === t.key && st.langOptionActive]}
                    onPress={() => { setConsultationType(t.key); setShowTypePickerModal(false); }}
                  >
                    <Text style={st.langFlag}>{t.icon}</Text>
                    <View style={st.langInfo}>
                      <Text style={[st.langName, consultationType === t.key && st.langNameActive]}>{t.label}</Text>
                    </View>
                    {consultationType === t.key && <CheckCircle size={18} color="#0033A0" />}
                  </TouchableOpacity>
                ))}
              </View>
            </TouchableOpacity>
          ...
          {step === 'ready' && (
            <View style={st.readyContainer}>
              <Text style={st.subtitle}>
                Record the doctor-patient conversation during this clinic visit. AI will transcribe, extract clinical insights, and structure a consultation summary.
              </Text>
              ...
              <View style={st.patientInfoCard}>
                <Text style={st.patientInfoTitle}>Patient Details (Optional)</Text>
                <View style={st.patientRow}>
                  <View style={st.patientField}>
                    <Text style={st.fieldLabelSmall}>PATIENT ID</Text>
                    <TextInput
                      style={st.patientInput}
                      value={patientId}
                  ...
                  <View style={st.patientField}>
                    <Text style={st.fieldLabelSmall}>PATIENT NAME</Text>
                    <TextInput
                      style={st.patientInput}
                      value={patientName}
              ...
              <TouchableOpacity style={st.bigMicBtn} onPress={startRecording} activeOpacity={0.7}>
                <View style={st.micCircle}><Mic size={36} color="#0033A0" /></View>
              </TouchableOpacity>
              <Text style={st.micLabel}>Tap to start recording</Text>

              <View style={st.pipelineInfo}>
                <Text style={st.pipelineTitle}>AI Processing Pipeline</Text>
                <View style={st.pipelineStep}><View style={[st.pipeDot, { backgroundColor: '#0033A0' }]} /><Text style={st.pipeText}>Record clinic visit conversation</Text></View>
                <View style={st.pipelineStep}><View style={[st.pipeDot, { backgroundColor: '#0033A0' }]} /><Text style={st.pipeText}>Transcribe with Whisper AI ({selectedLangInfo.name})</Text></View>
                <View style={st.pipelineStep}><View style={[st.pipeDot, { backgroundColor: '#0033A0' }]} /><Text style={st.pipeText}>Extract symptoms, diagnosis, medications</Text></View>
                <View style={st.pipelineStep}><View style={[st.pipeDot, { backgroundColor: '#0033A0' }]} /><Text style={st.pipeText}>Review & save consultation notes</Text></View>
              </View>
            </View>
          )}
          ...
          {step === 'recording' && (
            <View style={st.recordingContainer}>
              ...
              <Text style={st.langRecording}>{selectedLangInfo.flag} {selectedLangInfo.name} • {selectedTypeInfo.label}</Text>
          ...
          {step === 'transcribing' && (
            <View style={st.processingContainer}>
              ...
              <View style={st.processingSteps}>
                ...
                <Text style={st.procPending}>○ LLM clinical extraction</Text>
              </View>
            </View>
          )}
          ...
          {step === 'extracting' && (
            <View style={st.processingContainer}>
              <ActivityIndicator size="large" color="#0033A0" />
              <Text style={st.processingTitle}>Analyzing Conversation...</Text>
              <Text style={st.processingDesc}>LLM extracting clinical insights</Text>
              <View style={st.processingSteps}>
                <Text style={st.procStep}>✓ Conversation transcribed</Text>
                <Text style={[st.procStep, st.procActive]}>⟳ Extracting diagnosis, medications...</Text>
              </View>
            </View>
          )}
            ...
            <>
              ...
              {sttInfo && (
                <View style={st.sttStats}>
                  {sttInfo.language_name ? <View style={st.sttStat}><Globe size={14} color="#0033A0" /><Text style={st.sttStatText}>{sttInfo.language_name}</Text></View> : null}
                  {sttInfo.confidence ? <View style={st.sttStat}><BarChart3 size={14} color="#0033A0" /><Text style={st.sttStatText}>{(sttInfo.confidence * 100).toFixed(0)}% conf.</Text></View> : null}
                  {sttInfo.duration_seconds ? <View style={st.sttStat}><Clock size={14} color="#0033A0" /><Text style={st.sttStatText}>{sttInfo.duration_seconds}s audio</Text></View> : null}
                  <View style={st.sttStat}><Cpu size={14} color="#0033A0" /><Text style={st.sttStatText}>{sttInfo.model || `whisper-${whisperModel}`}</Text></View>
                </View>
              )}
              ...
              <View style={st.transcriptCard}>
                <View style={st.transcriptHeader}>
                  <Text style={st.transcriptLabel}>VISIT TRANSCRIPT</Text>
                  {!isEditing ? (
                    <TouchableOpacity style={st.editButton} onPress={startEditing}>
                      <Edit3 size={14} color="#0033A0" /><Text style={st.editButtonText}>Edit</Text>
                    </TouchableOpacity>
                  ) : (
              ...
              {extraction && (
                <View style={st.extractionContainer}>
                  <Text style={st.extractionTitle}>Clinical Extraction</Text>
              ...
              {/* Patient Info (if not set before recording) */}
              {!patientId && !patientName && (
                <View style={st.patientInfoCard}>
                  <Text style={st.patientInfoTitle}>Link Patient (Optional)</Text>
                  <View style={st.patientRow}>
                    <View style={st.patientField}>
                      <Text style={st.fieldLabelSmall}>PATIENT ID</Text>
                      <TextInput style={st.patientInput} value={patientId} onChangeText={setPatientId} placeholder="PAT-XXXXXX" placeholderTextColor={theme.textSecondary} autoCapitalize="characters" />
                    </View>
                    <View style={st.patientField}>
                      <Text style={st.fieldLabelSmall}>NAME</Text>
                      <TextInput style={st.patientInput} value={patientName} onChangeText={setPatientName} placeholder="Patient name" placeholderTextColor={theme.textSecondary} />
                    </View>
                  </View>
                </View>
              )}
              ...
              <View style={st.inputGroup}>
                <Text style={st.fieldLabel}>DOCTOR'S NOTES</Text>
                <TextInput
                  style={[st.input, { height: 80, textAlignVertical: 'top', paddingTop: 12 }]}
                  value={doctorNotes}
                  onChangeText={setDoctorNotes}
                  placeholder="Additional clinical observations, plan of care..."
                  placeholderTextColor={theme.textSecondary}
                  multiline
                />
              </View>
              ...
              {!isEditing && (
                <>
                  <TouchableOpacity style={st.submitBtn} onPress={saveConsultation} activeOpacity={0.8}>
                    <Save size={18} color="#FFF" />
                    <Text style={st.submitBtnText}>Save Consultation Notes</Text>
                  </TouchableOpacity>
          ...
          {step === 'submitting' && (
            <View style={st.processingContainer}>
              <ActivityIndicator size="large" color="#0033A0" />
              <Text style={st.processingTitle}>Saving Consultation...</Text>
              <Text style={st.processingDesc}>Encrypting and saving notes</Text>
            </View>
          )}
          ...
          {step === 'done' && (
            <View style={st.doneContainer}>
              <CheckCircle size={48} color="#0033A0" />
              <Text style={st.doneTitle}>Consultation Saved!</Text>
              ...
              <TouchableOpacity style={st.newCaseBtn} onPress={resetForm}>
                <Mic size={16} color="#0033A0" />
                <Text style={st.newCaseText}>Record Another Visit</Text>
              </TouchableOpacity>
            </View>
          )}
        </ScrollView>
...
const st = StyleSheet.create({
  ...
  aiBannerText: { fontSize: FontSizes.sm, color: '#0033A0', fontWeight: '500' },
  configRow: { flexDirection: 'row', gap: Spacing.sm, marginBottom: Spacing.lg },
  languageSelector: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border, borderRadius: 8, paddingHorizontal: Spacing.sm, paddingVertical: 10, gap: 6 },
  langText: { flex: 1, fontSize: FontSizes.sm, color: theme.textPrimary, fontWeight: '500' },
  typeSelector: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border, borderRadius: 8, paddingHorizontal: Spacing.sm, paddingVertical: 10, gap: 6 },
  typeIcon: { fontSize: 16 },
  typeText: { flex: 1, fontSize: FontSizes.sm, color: theme.textPrimary, fontWeight: '500' },
  ...
  retryBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#0033A0', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6, gap: 4 },
  retryText: { color: '#FFF', fontSize: FontSizes.sm, fontWeight: '600' },
  subtitle: { fontSize: FontSizes.md, color: theme.textSecondary, lineHeight: 22, marginBottom: Spacing.md },
  readyContainer: { alignItems: 'center', paddingVertical: Spacing.sm },
  patientInfoCard: { backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border, borderRadius: 10, padding: Spacing.base, marginBottom: Spacing.lg, width: '100%' },
  patientInfoTitle: { fontSize: FontSizes.sm, fontWeight: '700', color: theme.textSecondary, letterSpacing: 1, marginBottom: Spacing.md },
  patientRow: { flexDirection: 'row', gap: Spacing.sm },
  patientField: { flex: 1 },
  fieldLabelSmall: { fontSize: 10, fontWeight: '700', color: theme.textSecondary, letterSpacing: 1, marginBottom: 4 },
  patientInput: { backgroundColor: theme.inputBg, borderWidth: 1, borderColor: theme.border, borderRadius: 6, paddingHorizontal: Spacing.sm, height: 40, fontSize: FontSizes.sm, color: theme.textPrimary },
  bigMicBtn: { marginTop: Spacing.md },
  ...
  pipelineInfo: { backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border, borderRadius: 10, padding: Spacing.base, marginTop: Spacing.lg, width: '100%' },
  ...
  pipeDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#0033A0' },
  ...
  micCircleActive: { width: 96, height: 96, borderRadius: 48, backgroundColor: '#0033A0', alignItems: 'center', justifyContent: 'center' },
  waveContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 3, marginTop: Spacing.lg, height: 40 },
  waveBar: { width: 4, borderRadius: 2, backgroundColor: '#0033A0' },
  ...
  progressFill: { height: '100%', backgroundColor: '#0033A0', borderRadius: 2 },
  processingSteps: { marginTop: Spacing.lg, alignItems: 'flex-start' },
  procStep: { fontSize: FontSizes.md, color: '#0033A0', marginBottom: 4 },
  procActive: { color: '#0033A0', fontWeight: '600' },
  ...
  sttStatText: { fontSize: FontSizes.sm, color: '#0033A0', fontWeight: '600' },
  ...
  summaryCard: { backgroundColor: '#EFF6FF', borderWidth: 1, borderColor: '#BFDBFE', borderRadius: 10, padding: Spacing.base, marginBottom: Spacing.md },
  summaryLabel: { fontSize: FontSizes.xs, fontWeight: '700', color: '#1E40AF', letterSpacing: 1.5, marginBottom: Spacing.sm },
  summaryText: { fontSize: FontSizes.base, color: '#1E40AF', lineHeight: 22 },
  ...
  editButtonText: { fontSize: FontSizes.sm, color: '#0033A0', fontWeight: '600' },
  ...
  saveBtn: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 6, backgroundColor: '#0033A0' },
  ...
  transcriptInput: { fontSize: FontSizes.base, color: theme.textPrimary, lineHeight: 24, backgroundColor: theme.inputBg, borderWidth: 1, borderColor: '#0033A0', borderRadius: 6, padding: Spacing.md, minHeight: 140 },
  ...
  countBadge: { backgroundColor: '#0033A0', width: 22, height: 22, borderRadius: 11, alignItems: 'center', justifyContent: 'center' },
  ...
  submitBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#0033A0', height: 52, borderRadius: 9999, gap: Spacing.sm },
  submitBtnText: { color: '#FFF', fontSize: FontSizes.base, fontWeight: '700' },
  reRecordBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#0033A0', height: 48, borderRadius: 9999, gap: 6, marginTop: Spacing.md },
  reRecordText: { fontSize: FontSizes.base, color: '#0033A0', fontWeight: '600' },
