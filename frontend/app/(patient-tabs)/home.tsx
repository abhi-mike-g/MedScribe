...
import { Shield, Lock, ClipboardList, FileText, Activity, User, Copy, CheckCircle } from 'lucide-react-native';
...
export default function PatientHome() {
  ...
  const load = async () => {
    try {
      const r = await authFetch('/api/dashboard/stats');
    ...
    <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.scroll} refreshControl={<RefreshControl refreshing={false} onRefresh={load} />}>
        <View style={s.headerRow}>
          ...
          <View style={s.badge}><User size={20} color="#10B981" /></View>
        </View>

        <TouchableOpacity style={s.idCard} onPress={copyPatientId} activeOpacity={0.7}>
          <View style={s.idLeft}>
            <Text style={s.idLabel}>YOUR PATIENT ID</Text>
            <Text style={s.idValue}>{stats?.patient_id || user?.patient_id || 'N/A'}</Text>
            <Text style={s.idHint}>Share this ID with your doctor</Text>
          </View>
          {copied ? <CheckCircle size={20} color="#10B981" /> : <Copy size={20} color={theme.textSecondary} />}
        </TouchableOpacity>

        <View style={s.e2ee}>
          <Lock size={12} color="#10B981" />
          <Text style={s.e2eeT}>E2EE Active — {stats?.encryption_status || 'AES-256-GCM'}</Text>
        </View>
        ...
        <View style={s.statsRow}>
          ...
          <View style={s.stat}>
            ...
            <Text style={s.statNum}>{stats?.prescription_count || 0}</Text>
            <Text style={s.statLabel}>Rx</Text>
          </View>
        </View>

        <TouchableOpacity style={s.recordBtn} onPress={() => router.push('/(patient-tabs)/record')} activeOpacity={0.8}>
          <Text style={s.recordBtnText}>Record New Case</Text>
        </TouchableOpacity>
        ...
        {stats?.recent_cases?.length ? stats.recent_cases.map((c: any) => (
          <TouchableOpacity key={c.id} style={s.card} onPress={() => router.push(`/case/${c.id}`)}>
            <View style={s.cardLeft}>
              ...
              <View>
                <Text style={s.cardTitle}>{c.chief_complaint || 'Case Submission'}</Text>
                <Text style={s.cardMeta}>{new Date(c.created_at).toLocaleDateString()}</Text>
              </View>
            </View>
          ...
          <View style={s.empty}>
            <Text style={s.emptyT}>No cases yet</Text>
            <Text style={s.emptyS}>Tap "Record New Case" to get started</Text>
          </View>
        )}

        <View style={s.compliance}>
          <Shield size={14} color="#10B981" />
          <Text style={s.complianceT}>HIPAA & GDPR Compliant • Patient Portal</Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  badge: { width: 44, height: 44, borderRadius: 12, backgroundColor: '#ECFDF5', alignItems: 'center', justifyContent: 'center' },
  idCard: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#052E16', borderRadius: 12, padding: Spacing.base, marginBottom: Spacing.md },
  idLeft: { flex: 1 },
  idLabel: { fontSize: FontSizes.xs, fontWeight: '700', color: '#6EE7B7', letterSpacing: 1.5 },
  idValue: { fontSize: FontSizes.xxl, fontWeight: '800', color: '#FFFFFF', letterSpacing: 2, marginTop: 4 },
  idHint: { fontSize: FontSizes.xs, color: '#6EE7B7', marginTop: 4 },
  ...
  recordBtn: { backgroundColor: '#10B981', height: 52, borderRadius: 9999, alignItems: 'center', justifyContent: 'center', marginBottom: Spacing.xl },
  recordBtnText: { color: '#FFF', fontSize: FontSizes.base, fontWeight: '700' },
  ...
  cardTitle: { fontSize: FontSizes.base, fontWeight: '600', color: theme.textPrimary },
  ...
  emptyT: { fontSize: FontSizes.base, fontWeight: '600', color: theme.textSecondary },
  emptyS: { fontSize: FontSizes.sm, color: theme.textSecondary, marginTop: 4 },
